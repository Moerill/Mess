...
async function preCreateChatMessageHook(data) {
	...
	if (!btn) btn = div.querySelector('button[data-action="damage"]');

	if (btn) renderAttack({ currentTarget: btn });
}
...
async function renderAttack(ev) {
	...
	for (const target of targets) {
		...
		if (targetActor) {
			...
			let di = targetActor.data.traits.di.value
				.filter((e) => e !== 'custom')
				.map((e) => game.i18n.localize('DND5E.Damage' + e[0].toUpperCase() + e.substring(1)));
			if (targetActor.data.traits.di.custom) di.push(targetActor.data.traits.di.custom);
			targetData.di = {
			...
			let dr = targetActor.data.traits.dr.value
				.filter((e) => e !== 'custom')
				.map((e) => game.i18n.localize('DND5E.Damage' + e[0].toUpperCase() + e.substring(1)));
			if (targetActor.data.traits.dr.custom) dr.push(targetActor.data.traits.dr.custom);
			targetData.dr = {
				label: '<i class="mess-icon dam-res"></i>',
				title: game.i18n.localize('DND5E.DamRes'),
				value: dr.join(', '),
			};
			let dv = targetActor.data.traits.dv.value
				.filter((e) => e !== 'custom')
				.map((e) => game.i18n.localize('DND5E.Damage' + e[0].toUpperCase() + e.substring(1)));
			if (targetActor.data.traits.dv.custom) dv.push(targetActor.data.traits.dv.custom);
			targetData.dv = {
		...
		const displayName =
			target.data.displayName === CONST.TOKEN_DISPLAY_MODES.ALWAYS || target.data.displayName === CONST.TOKEN_DISPLAY_MODES.HOVER;
		...
		if (autoroll.hit || autoroll.dmg) html = await autoRoll(autoroll, html, rollMode);
...
async function autoRoll(autoroll, template, rollMode) {
	...
	if (autoroll.hit) {
		...
		if (toHitBtn) {
			...
			if (dsn) await game.dice3d.showForRoll(r, game.user, true, whispers, blind);
		}
	}
	...
	if (dsn) await game.dice3d.showForRoll(rolls, game.user, true, whispers, blind);
	else AudioHelper.play({ src: CONFIG.sounds.dice }, true);
...
function chatListeners(html) {
	if (!html) html = $(document.getElementById('chat-log'));
...
async function onChatCardAction(ev) {
	ev.preventDefault();
	ev.stopPropagation();
	if (ev.currentTarget.dataset.action === 'attack') return renderAttack(ev);
	if (ev.currentTarget.dataset.action === 'damage') return renderAttack(ev);
	if (ev.currentTarget.dataset.placeTemplate) return renderTemplate(ev);
